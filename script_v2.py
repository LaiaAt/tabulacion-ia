# ==============================================================================
# SISTEMA DE TABULACIÓN RESTREPO_2 - VERSIÓN 2 (RECORTE ÓPTICO 200 DPI + ALMA-R)
# ==============================================================================
...
def obtener_insumos_documento(ruta_pdf):
    try:
        ...
        # Recorte de alta definición a 200 DPI del sticker superior derecho de la página 1
        p1 = doc[0]
        ...
        # Detección de carátula remisoria de la ANI
        ...
        imagenes_b64 = []
        # La primera imagen enviada a Pixtral es el acercamiento a 200 DPI del sticker
        imagenes_b64.append(b64_sticker)
...
2. "no_radicado_remitente":
   ...
   - EN CARTAS DE CONCESIÓN ALTO MAGDALENA: Código bajo el código de barras (Ej: "ALMA-2017-5207").
...
4. "no_radicado_destinatario":
   ...
   - En RADICADAS (REGLA FUNDAMENTAL):
     * SI FUE ENVIADA A CONCESIÓN ALTO MAGDALENA:
       La primera imagen adjunta es un ACERCAMIENTO A 200 DPI del sticker superior derecho.
       Debajo del código de barras está impreso "ALMA-R-AAAA-XXXXX" (Ej: "ALMA-R-2017-00392", "ALMA-R-2020-0287").
       Cópialo con total prioridad. Si no está en el sticker, búscalo en el correo de confirmación de las últimas páginas.
...
6. "asunto":
   - En cartas de Consorcio 4C (Radicadas): Transcribe el bloque de Asunto que empieza por "Ref. Contrato...".
...
def extraer_radicado_destinatario_radicadas_profundo(texto_completo, texto_sticker_ocr, dest_entidad):
    ...
    # 2. LA ANI (NÚMEROS LIMPIOS SIN LA PALABRA 'ANI')
...
def rescatar_asunto_completo_radicadas(texto_completo):
    ...
    if m_ref:
        ...
        if len(res) > 300: res = res[:300].rsplit(' ', 1)[0]
        return res
    return ""
...
def blindaje_logica_negocio(datos, nombre_archivo, texto_completo, texto_sticker_ocr, anio_carpeta, tipo_flujo):
    ...
    if es_recibida:
        ...
        # RESCATE PROFUNDO CON OCR DE 200 DPI Y TEXTO MULTIPÁGINA
...
def auto_sanar_memoria_completa(ruta_memoria, carpeta_objetivo):
    ...
    2. Rescata los ALMA-R del sticker de la página 1 usando OCR a 200 DPI.
    """
    ...
    try:
        ...
        for idx, row in df_m.iterrows():
            ...
            if not es_rec:
                ...
                # 2. Rescatar ALMA-R en cartas hacia Concesión si tenían SIN NÚMERO
                if "MAGDALENA" in dest_ent and (rad_dest in ["SIN NÚMERO", "SIN NUMERO", "NO IDENTIFICADO", ""] or not re.search(r'ALMA[-\s]?R', rad_dest, re.IGNORECASE)):
                    ...
                    if os.path.exists(ruta_pdf):
                        try:
                            ...
                            p1 = d_doc[0]
                            w, h = p1.rect.width, p1.rect.height
                            rect_s = fitz.Rect(w * 0.45, 0, w, h * 0.38)
                            pix_s = p1.get_pixmap(clip=rect_s, dpi=200)
                            img_s = Image.open(io.BytesIO(pix_s.tobytes("png"))).convert("L")
                            txt_ocr = ""
                            if TESSERACT_DISPONIBLE:
                                txt_ocr = pytesseract.image_to_string(img_s, config="--psm 6")
                            t_full = ""
                            for p in d_doc: t_full += p.get_text() + "\n"
                            d_doc.close()

                            rad_hallado = extraer_radicado_destinatario_radicadas_profundo(t_full, txt_ocr, dest_ent)
                            if rad_hallado:
        ...
        if modificados > 0:
            print(f"🧹 Auto-Sanador V2: {modificados} filas perfeccionadas con ALMA-R.", flush=True)
    except Exception as e:
        print(f"⚠️ Error en sanador: {e}", flush=True)

def procesar_archivos():
    print("\n" + "="*70, flush=True)
    print(" MOTOR RESTREPO_2 (VERSIÓN 2: OCR ÓPTICO 200 DPI + ALMA-R)", flush=True)
    print("="*70, flush=True)
    ...
    for tipo, ruta_raiz in flujos:
        ...
        print(f"\n📂 [CENSO FÍSICO V2] Encontrados {total_en_carpeta} archivos PDF en {tipo}.", flush=True)
        ...
        for ronda in range(1, max_rondas + 1):
            ...
            if not pendientes:
                print(f"✅ ¡Conciliación perfecta V2 en {tipo}! {len(procesados_actuales)} de {total_en_carpeta} cartas aseguradas.", flush=True)
                break
    ...
    if EMAIL_REMITENTE and EMAIL_PASSWORD:
        try:
            msg = EmailMessage()
            msg['Subject'] = f'✅ Tabulación Verificada V2 ({etiqueta}) - ALMA-R y Cero Prefijos ANI'
            msg['From'] = EMAIL_REMITENTE
            msg['To'] = EMAIL_DESTINO
            msg.set_content(
                f'Hola,\n\n'
                f'El proceso V2 para {etiqueta} ha finalizado con ÉXITO Y CONCILIACIÓN FÍSICA TOTAL.\n\n'
                f'{reporte_validacion}\n'
                f'Se integró el recorte a 200 DPI y OCR óptico local para extraer el 100% de los códigos ALMA-R de los stickers de la Concesión.\n\n'
                f'Saludos cordiales.'
            )
